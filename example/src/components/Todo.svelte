<script lang="ts">
  ...
  function handleEdit(event: KeyboardEvent) {
    ...
    else if (event.key === 'Esc') muts.setEditingTodo(null);
  }
</script>

<li class:completed={$todo.completed} class:editing={$todo.id === $editingTodoId}>
  <div class="view">
    <input id="todo-input" class="toggle" type="checkbox" on:change={() => toggleTodo($todo.id)} checked={$todo.completed}>
    <label for="todo-input" on:dblclick={showInput}>{$todo.description}</label>
